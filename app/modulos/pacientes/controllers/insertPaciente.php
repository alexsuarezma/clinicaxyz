...
if($paciente>0 || $empleado>0){
  echo false;
    ...
    $username = "$name $apellidoPaterno";
    $sql = "INSERT INTO usuario (username, password) VALUES (:username,:password)";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':username', $username);
    //CAMBIAR LA CONTRASEÑA BASE FORMATO // asuarez9999999999  (primeraletradeprimer nombre, primerapellido, diay año en el que se registra)
    $cadena = trim(utf8_decode($apellidoPaterno));
    $autogenerate = strtolower( substr( $name,0,1 ) ).strtolower( $cadena ).$cedula;
    $passwordBcrypt = password_hash($autogenerate, PASSWORD_BCRYPT);
